...
def load_model_from_config(config, ckpt, verbose: bool = False, device="cuda:0"):
    """
    ...
    :param device: Target device, e.g. "cuda:0", "cuda:1", or "cpu".
    :returns: A SDM model.
    """
    ...
    # Keep LightningModule.device property in sync with the actual device
...
class target_model(nn.Module):
    ...
    def __init__(self, model,
                 ...
                 mode: int = 2, 
                 rate: int = 10000, g_mode='+', device="cuda:0"):
        """
...
def init(epsilon: int = 16, steps: int = 100, alpha: int = 1,
         ...
         ckpt: str = None, base: str = None, mode: int = 2, rate: int = 10000, g_mode='+', device="cuda:0", input_prompt='a photo'):
    """
...
def infer(img: PIL.Image.Image, config, tar_img: PIL.Image.Image = None, diff_pgd=None, using_target=False, device="cuda:0") -> np.ndarray:
    """
...
def main(cfg : DictConfig):
    ...
    device=args.device
    # Convert device to torch device string (e.g., int 0 -> "cuda:0", str "cuda:1" -> "cuda:1")
